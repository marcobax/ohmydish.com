...
class adminController extends Controller
{
    ...
    public function recipe_index()
    {
        ...
        $this->set([
            ...
            'recipes'    => $this->recipe_model->getRecords($where,$this->getSortOrder('published', 'desc'), $this->getPagination()),
            'pagination' => $this->getPagination(),
            'query'      => $query
        ]);
    ...
    public function recipe_edit($id)
    {
        ...
        if (is_array($post_values) && count($post_values)) {
            ...
            $values = [
                ...
                'displaytime' => $post_values['displaytime'],
                'yield' => $post_values['yield'],
    ...
    public function sure(string $model, $id, $sure = false)
    {
        if (true === $sure || (!is_bool($sure) && strlen($sure) )) {
            if ('yup' === $sure) {
                ...
                $_SESSION['flash'] = ['type' => 'info', 'message' => 'Oho! You chickin!<br><img src="' . Core::asset('img/chickin.gif') . '"><br>Nee maar serieus goed dat je niet alles meteen verwijderd hebt!'];
            }
