...
$(document).ready(function(){
    ...
    $('.autosave').on('keyup focus', delay(function(){
        var self = $(this);
        $.ajax({
            url: $(this).data('save-url'),
            type: 'POST',
            ...
            success: function(result) {
                ...
                } else if ('wrong-user' === result) {
                    window.location.replace('https://www.youtube.com/watch?v=Tlt_xm9APMM');
                } else if('not-logged-in' === result) {
                    window.location.replace($('body').data('login-url'));
                }
            }
    ...
    $('.select-avatar').on('click', function(){
        $('.select-avatar.bg-danger').removeClass('bg-danger');
